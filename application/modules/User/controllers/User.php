...
class User extends  Public_Controller
{
    function __construct() {
        ...
        $this->no_of_groups = $this->aauth->get_number_of_videos();
...
//
    ...
    public function user_list($msg=0){
        ...
        $no_of_groups = $this->aauth->get_number_of_videos();
    ...
    public function delete_video($group_id=FALSE){

        if(!$group_id)
            $group_id = $this->input->get('gid');

        $this->aauth->delete_video($group_id);
        redirect('/user/user_videos', 'refresh');
    ...
    //   USER GROUPS              //
    //*****************************//
    public function user_videos($msg=0){
        ...
        $data['groups'] = $this->aauth->list_videos($this->GroupperPage,$offset);
        $data['subnav'] = $this->load->view('menu/subnav', $subnav_data, TRUE);
        $data['menu'] = $this->load->view('menu/admin_main', $menu_data, TRUE);
        ...
        //pagination configuration
        $config['base_url']    = base_url().'user/list_videos';
