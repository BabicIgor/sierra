...
class Aauth {
	 ...
	 */
	public function create_user($email, $pass, $firstname, $lastname, $username = FALSE) {
	 ...
	 * @return bool Update fails/succeeds
	 */
	public function update_user($user_id, $email = FALSE, $pass = FALSE,$firstname, $lastname, $username = FALSE) {
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE, $sort = FALSE) {
		...
		if ($group_par != FALSE) {
			...
			$this->aauth_db->select('*')
				->from($this->config_vars['real_users']);
	...
	/**
	 * Get user id
	 * Get user id from username, if par. not given, return current user's id
	 * @param string|bool $usernam Username for user
	 * @return int User id
	 */
	...
	public function get_user_first_name($user_id = FALSE) {
	...
	public function get_user_last_name($user_id = FALSE) {
	...
	/**
	 * Get initials of First and Last Names
	 * Return string
	 * @param int $user_id ; if not given current user
	 * @return User's name initials
	 */

	public function get_user_name_initials($user_id = FALSE){
		if ($user_id == FALSE)
			$user_id = $this->CI->session->userdata('id');

		if($user_id==FALSE){return FALSE;}

		$fname = $this->get_user_first_name($user_id);
		$lname = $this->get_user_last_name($user_id);
	...
	public function get_number_of_users(){
		...
		return count($this->list_users());

	}

	public function get_number_of_banned_users() {
	...
	public function get_user_pass($user_id = FALSE) {
	...
	public function get_number_of_groups(){
		return count($this->list_groups());
	}
	public function get_number_of_videos(){
		return count($this->list_videos());
	}
	...
	public function is_the_only_admin($user_id = FALSE ) {
	 ...
	 */
	public function list_groups( $limit = FALSE, $offset = FALSE) {
	...
	{
		$this->aauth_db->where('id', $video_id);
		return $this->aauth_db->delete($this->config_vars['videos']);

	}
	public function list_videos( $limit = FALSE, $offset = FALSE) {
		...
		$query = $this->aauth_db->get($this->config_vars['videos']);
		return $query->result();
	}
	 ...
	 * @return bool
	 */
	public function is_allowed($perm_id, $user_id=FALSE){
	 ...
	 * @return bool
	 */
	public function is_group_allowed($perm_id,$group_id){
	...
	public function get_all_methods ($controller=0) {
		if($controller > 0)
			$query = $this->aauth_db->where('controller_id', $controller);

		$query = $this->aauth_db->get('aauth_perms_methods');
	...
	public function get_controller_id($controller){
		$query = $this->aauth_db->where('controller', $controller);
		$query = $this->aauth_db->get('aauth_perms_controller');
	...
	public function user_has_perms($user_id){
	...
	public function delete_user_perms($user_id,$perm_id) {
	...
	public function get_user_perms($user_id,$controller_id){

		$query = $this->aauth_db->select('perm_id,controller_id');

		$query = $this->aauth_db->where('user_id', $user_id);
		$query = $this->aauth_db->where('controller_id', $controller_id);

		$query = $this->aauth_db->join('aauth_perms_methods', "id = perm_id");
	...
	public function get_group_perms($group_id,$controller_id){

		$this->aauth_db->select('perm_id,controller_id');

		$this->aauth_db->where('group_id', $group_id);
		$this->aauth_db->where('controller_id', $controller_id);

		$this->aauth_db->join('aauth_perms_methods', "id = perm_id");
	...
	public function add_user_perms($user_id,$perm_id){
	...
	public function add_group_perms($group_id,$perm_id){
	...
	public function delete_group_perms($group_id, $perm_id) {
	...
	public function group_has_perms($group_id){
	...
	public function get_controller_names(){
		...
		$this->CI->db->distinct();
		$this->CI->db->select('controller_id, controller');
		$this->CI->db->from('aauth_perms_methods');
		$this->CI->db->join('aauth_perms_controller', 'aauth_perms_controller.id = aauth_perms_methods.controller_id');
	 ...
	 */
	public function get_perm_id($perm_method,$controller_id =FALSE) {

		if($controller_id){
			$this->aauth_db->where('controller_id', $controller_id);
		}
		$this->aauth_db->where('methods', $perm_method);
		$query = $this->aauth_db->get($this->config_vars['perms']);
	...
	public function is_user_allowed($url){
		$section = explode('/',$url);
		$controller = $section[0];
		$controller_id =$this->get_controller_id($controller);
		if(array_key_exists(1,$section)){
			$method = $section[1];
		}else{
			$method = 'index';
		}
		$user_groups = $this->get_user_groups();
		$perm_id = $this->get_perm_id($method,$controller_id);
		foreach($user_groups as $user_group){
			if($this->is_group_allowed($perm_id,$user_group->group_id)){
				return TRUE;
			}
		}
